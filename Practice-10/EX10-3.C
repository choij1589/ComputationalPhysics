...
int main() {
	...
	for (unsigned long i = 0; i < max_iter; i++)
		hist[int((gaus[i]-min)/(max-min)*nbins)]++;

	// now calculate standard deviation
	double mean = 0.;
	for (unsigned long i = 0; i < max_iter; i++)
		mean += gaus[i]/max_iter;
	double var = 0.;
	for (unsigned long i = 0; i < max_iter; i++)
		var += pow(gaus[i] - mean, 2)/max_iter;
	double std = sqrt(var);
